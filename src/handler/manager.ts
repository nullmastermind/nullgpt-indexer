...
import { pathExists, pathExistsSync, readFile, readdir } from 'fs-extra';
...
const managerHandler = async (req: Request, res: Response) => {
  ...
  await Promise.all(
    (await readdir(docDir)).map(async (f) => {
  ...
  const findGit = async (findPath: string): Promise<string | undefined> => {
    ...
    const currentDir = (await isDirectory(findPath)) ? findPath : path.dirname(findPath);
